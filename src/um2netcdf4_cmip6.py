...
#
# Singleton dimensions are eliminated,
...
def process(ifile, ofile, args):
    ...
    def apply_mask(var,heavyside):
        ...
        if var.shape == heavyside.shape:
            var = MV.where(np.greater_equal(heavyside,args.hcrit),var/heavyside,fVal)
            ...
            if vshape == hshape:
                ...
                if set(vlevs).issubset(set(hlevs)):
                    ...
                    for k in range(len(vlevs)):
                        ...
                        newvar[k] = MV.where(np.greater_equal(heavyside[kh],args.hcrit),newvar[k]/heavyside[kh],fVal)
    ...
    try:
        ...
        print("Error opening file", ifile, file=sys.stderr)
        sys.exit(1)
    ...
    fo = netCDF4.Dataset(ofile,'w',format=ncformats[args.nckind])

    history = "%s converted to netCDF by %s on %s." % (os.path.abspath(ifile), os.getenv('USER'),datetime.datetime.now().strftime("%Y-%m-%d"))
...
if __name__ == '__main__':
    ...
    process(args.ifile, args.ofile, args)
