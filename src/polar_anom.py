...
if f.fixhd[FH_HorizGrid] != 0:
    print("Error - not a global grid: FH_HorizGrid=", f.fixhd[FH_HorizGrid])
    sys.exit(1)
...
for k in range(f.fixhd[FH_LookupSize2]):
    ...
    if anom_sp > 0 or anom_np > 0:
        print("Error - polar anomaly: Field %d Level %d Index %d" % (ilookup[ITEM_CODE], ilookup[LBLEV], k))
        rel_sp = rel_np = 0.
        if anom_sp > 0:
            rel_sp =  anom_sp/data[0].mean()
        if anom_np > 0:
            rel_np =  anom_np/data[-1].mean()
        print("   Absolute value (SP, NP):", anom_sp, anom_np)
        print("   Relative value (SP, NP):", rel_sp, rel_np)
